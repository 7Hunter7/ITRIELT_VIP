...
- Проект развернут на Vercel: [https://itrielt-vip.ru/](https://itrielt-vip.netlify.app/)
...
- **`isMobile()`:** Определяет, запущено ли приложение на мобильном устройстве, основываясь на ширине экрана.

### Функции для UI

- **`toggleStatusBarMobile()`:** Переключает класс `status-bar--mobile` для затемнения статус-бара на мобильных устройствах.

### Функции для сортировки

- **`sortCards(sortBy)`:**
  - Сортирует массив `cardData` в зависимости от выбранного критерия (`sortBy`).
  - **Параметры:**
    - `sortBy` (string): Критерий сортировки (`price-asc`, `price-desc`, `area-asc`, `area-desc`).
...
│ │ ├── _header.scss
│ │ ├── _main__top.scss
│ │ ├── _multiple-select.scss # Стили для компонента множественного выбора
...
- Добавление фильтрации по различным параметрам.
- Реализация функциональности добавления объектов в подборки.
- И другие улучшения.
